...
const Payment = (props) => {
  ...
  const storeDataInDatabase = async (paymentArray, netAmountOwed1 , tripName ) => {
    try {
        const existingPayment = await Payment.findOne({ tripName }); // Check if the tripName exists

        if (existingPayment) {
            // If the trip already exists, update the existing record
            await Payment.findOneAndUpdate(
                { tripName }, // Find the document with the matching tripName
                {
                    $set: {
                        paymentArray, // Replace the paymentArray with the new one
                        netAmountOwed: Object.fromEntries(netAmountOwed1), // Update netAmountOwed
                    },
                },
            ...
            // If the tripName doesn't exist, create a new entry
            const newPayment = new Payment({
                tripName,
                paymentArray,
                netAmountOwed: Object.fromEntries(netAmountOwed1),
            });
